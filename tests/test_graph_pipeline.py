...
class PipelineTests(unittest.TestCase):
    def test_build_graph_artifacts_schema(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(metrics["collision_pair_radius_scale"], 1 + metrics["physics_config"]["COLLISION_RADIUS"])
            self.assertEqual(metrics["collision_base_pair_padding"], 2 * metrics["bubble_base_radius"] * metrics["physics_config"]["COLLISION_RADIUS"])
            self.assertEqual(metrics["collision_base_pair_radius"], 2 * metrics["bubble_base_radius"] * metrics["collision_pair_radius_scale"])
            self.assertEqual(metrics["collision_padding"], metrics["collision_base_pair_padding"])
            ...
            self.assertIn("COLLISION_FORCE_CUTOFF_MULTIPLIER", metrics["physics_config"])
            self.assertIn("VELOCITY_DECAY_START", metrics["physics_config"])
            self.assertIn("VELOCITY_DECAY_END", metrics["physics_config"])
            self.assertEqual(metrics["collision_force_cutoff_multiplier"], metrics["physics_config"]["COLLISION_FORCE_CUTOFF_MULTIPLIER"])
            self.assertEqual(metrics["collision_force_cutoff_enabled"], metrics["physics_config"]["COLLISION_FORCE_CUTOFF_MULTIPLIER"] > 0)
            self.assertEqual(metrics["collision_force_max"], metrics["collision_force_step"] * metrics["collision_force_cutoff_multiplier"])
            self.assertEqual(metrics["velocity_decay_start"], metrics["physics_config"]["VELOCITY_DECAY_START"])
            self.assertEqual(metrics["velocity_decay_end"], metrics["physics_config"]["VELOCITY_DECAY_END"])
            self.assertEqual(metrics["spring_force_step"], metrics["physics_config"]["SPRING_FORCE_STRENGTH"] * metrics["base_spacing"])
            self.assertEqual(metrics["max_collision_pair_radius"], 2 * metrics["max_bubble_radius"] * metrics["collision_pair_radius_scale"])
